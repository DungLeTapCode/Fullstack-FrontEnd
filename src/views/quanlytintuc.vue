...
const addBlog = async () => {
  ...
  try {
    const response = await axios.post('https://fullstack-backend-kiy9.onrender.com/api/Blogs/AddNewBlog', formData, {
      headers: {
...
const updateBlog = async () => {
  ...
  try {
    await axios.put(`https://fullstack-backend-kiy9.onrender.com/api/Blogs/UpdateBlog/${parseInt(editBlog.value.id)}`, formData, {
...
const deleteBlog = async (id) => {
  ...
  if (isConfirmed) {
    try {
      await axios.delete(`https://fullstack-backend-kiy9.onrender.com/api/Blogs/DeleteBlog/${id}`);
